...
help: ## Show this help message
	@echo "$(BLUE)============================$(NC)"
	@echo "$(BLUE)= Terraform CI/CD Makefile =$(NC)"
	@echo "$(BLUE)============================$(NC)"
	@echo ""
	...
	@echo "  PROJECT_NAME   Project name (default: $(PROJECT_NAME))"
	@echo ""
	@echo "$(GREEN)✅ Parameter names use lowercase for simplicity$(NC)"
	@echo "  ✅ Correct: make plan site=oci-labs environment=dev"
	@echo "  ✅ Example: make plan site=oci-tfstate workspace=default"
	@echo ""
	@echo "Site management:"
	@echo "  make list-sites         - List available sites"
	@echo "  make validate-site      - Validate current site exists"
	@echo "  make discover-options   - Discover all available sites and workspaces"
	@echo "  make discover-workspaces - Discover workspaces for current site"
	@echo "  make validate-options   - Validate current site and workspace"

# =============================================================================
# PRE-COMMIT CHECKS (mimics terraform-pre-commit.yml)
# =============================================================================
...
.PHONY: plan
plan: ## Generate Terraform plan for current environment
	...
	terraform -chdir=terraform/sites/$(site) workspace select $(workspace) || terraform -chdir=terraform/sites/$(site) workspace new $(workspace) && \
	terraform -chdir=terraform/sites/$(site) plan -var-file=terraform.tfvars -var-file=workspaces/$(workspace).tfvars -out=$$(mktemp).$(PROJECT_NAME).$(environment).$(workspace).tfplan 2>&1 | sed 's|terraform apply|terraform -chdir=terraform/sites/$(site) apply|g'
...
.PHONY: output-raw
output-raw: ## Show Terraform outputs in raw format
	$(call validate-parameters)
	@$(MAKE) validate-site
...
# =============================================================================
# DOCUMENTATION (mimics terraform-docs.yml)
# =============================================================================

.PHONY: docs
docs: ## Generate Terraform documentation for all modules and sites
...
# =============================================================================
# LINTING (mimics terraform-lint.yml)
# =============================================================================
...
# =============================================================================
# SECURITY SCANNING (mimics terraform-security.yml)
# =============================================================================

.PHONY: security-scan
security-scan: ## Run security scans (TFsec and Checkov)
...
list-sites: ## List available sites
	@echo "$(BLUE)Available Terraform sites:$(NC)"
...
show-plan: ## Show the most recent plan file for current site and workspace
	...
	@latest_plan=$$(find terraform/sites/$(SITE) -name "*$(PROJECT_NAME).$(ENVIRONMENT).$(WORKSPACE).tfplan" -type f -printf '%T@ %p\n' 2>/dev/null | sort -n | tail -1 | cut -f2- -d" ") && \
...
check-prerequisites: ## Check if all required tools are installed
	...
	if [ "$${#missing[@]}" -eq 0 ]; then \
		...
		echo "$(BLUE)Run 'make install-tools' to install missing tools$(NC)"; \
		exit 1; \
	fi
...
backup: ## Backup Terraform state
	...
	@mkdir -p backups
	@BACKUP_DIR="backups/$(PROJECT_NAME)-$(ENVIRONMENT)-$(WORKSPACE)-$$(date +%Y%m%d-%H%M%S)" && \
	mkdir -p "$$BACKUP_DIR" && \
...
# Note: Legacy targets are now handled by the main targets above
# The following targets are available for backward compatibility:
# - lint (runs TFLint)
# - security-scan (runs TFsec and Checkov)
